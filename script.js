...
function initClock() {
    ...
    const update = () => {
        ...
        if (clockEl) {
            clockEl.innerHTML =
                `<i class="fa-regular fa-clock"></i> ${now.toLocaleDateString('en-GB')} | ${now.toLocaleTimeString()}`;
        }
    };
...
function switchUserRole(role) {
    ...
    // ==========================================
    // IMPORTANT:
    // Add New Room is ADMIN ONLY.
    // Guest never sees this button.
    // ==========================================
...
function togglePaymentDetails() {
    ...
    if (method === 'bkash') {
        instructions.innerHTML =
            "<b>bKash Merchant Payment:</b> Send money to <code>01700000000</code> with your booking ID.";
    } else if (method === 'nagad') {
        instructions.innerHTML =
            "<b>Nagad Merchant Payment:</b> Send money to <code>01800000000</code>.";
    } else {
        instructions.innerHTML =
            "<b>Online Gateway:</b> You will be redirected to complete secure card payment.";
    }
}

// ==========================================
// 15. BOOKING SUBMISSION
// ==========================================
...
function handleBookingSubmit(event) {
    ...
    // Guest can only book available rooms.
...
function bookRoomFromBrowse(roomId) {
    ...
    // Guest can book ONLY available rooms.
    ...
    if (roomSelect) {
        ...
        const optionExists = Array.from(roomSelect.options).some(
    ...
    if (currentRole === 'guest') {
        ...
        if (guestNameField && currentUser.name) {
            guestNameField.value = currentUser.name;
        }

        if (guestEmailField && currentUser.email) {
            guestEmailField.value = currentUser.email;
        }

        if (guestPhoneField && currentUser.phone) {
            guestPhoneField.value = currentUser.phone;
        }
    }
    ...
    if (!bookingTabFound) {
        ...
        const availableTab = possibleTabs.find(
...
function renderDashboard() {
    ...
    if (totalRevEl) {
        totalRevEl.textContent = `৳${totalRev.toLocaleString()}`;
    }
...
function renderHousekeeping() {
    ...
    const cleanCount =
        roomList.filter(r => r.status === 'available').length;

    const dirtyCount =
        roomList.filter(r => r.status === 'dirty').length;

    const maintCount =
        roomList.filter(r => r.status === 'maintenance').length;
...
function promptAddNewRoom() {
    ...
    if (roomList.some(room => room.id === cleanId)) {
    ...
    const price = parseFloat(
        prompt("Enter Room Price per night (BDT):")
    );
...
path = "/mnt/data/grand_palace_updated.js"
